#https://github.com/kwyz/FaceRecognition-Python-
import os                                               # importing the OS for path
import cv2                                              # importing the OpenCV library
import numpy as np                                      # importing Numpy library
from PIL import Image                                   # importing Image library

# erro: module 'cv2.cv2' has no attribute 'face' veja a versão do python
#print(cv2.__version__)  # se python >=3  execute-> pip install opencv-contrib-python
...
def getImageWithID (path):
    ...
    for imagePath in imagePaths:
        ...
        ID = int(os.path.split(imagePath)[-1].split('.')[1])    # Retreave the ID of the array - Recupera o ID da matriz
        FaceList.append(faceNP)                         # Append the Numpy Array to the list
        IDs.append(ID)                                  # Append the ID to the IDs list
        cv2.imshow('Training Set: '+str(ID), faceNP)              # Show the images in the list
        cv2.waitKey(1)
