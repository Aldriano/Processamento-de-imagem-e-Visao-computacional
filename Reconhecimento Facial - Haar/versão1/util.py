...
now_time = time.time()    #time.clock()

face = cv2.CascadeClassifier('cascades/haarcascade_frontalcatface.xml')
glass_cas = cv2.CascadeClassifier('cascades/haarcascade_eye_tree_eyeglasses.xml')

WHITE = [255, 255, 255]

def FileRead():
    Info = open("NamesDB.txt", "r")                       # Open th text file in readmode
...
def AddName():
    if sys.version_info.major == 2:
        ...
        # permissão
    try:    
        Info = open("NamesDB.txt", "r+")
    except FileNotFoundError:
        Info = open("NamesDB.txt", "w+")
...
def DispID(x, y, w, h, NAME, Image):
    ...
    Name_y_pos = int(y - 10)
    Name_X_pos = int(x + w/2 - (len(NAME)*7/2))
 ...
 #  ------------------------------------    THE DRAWING OF THE BOX AND ID   --------------------------------------
    ...
    # error: (-5:Bad argument) in function 'rectangle' -  Can't parse 'pt1'. Sequence item with index 0 has a wrong type            
    ...
    cv2.putText(Image, NAME, (Name_X_pos, Name_y_pos - 10), cv2.FONT_HERSHEY_SIMPLEX, .4, WHITE)                         # Print the name of the ID
    
    #cv2.FONT_HERSHEY_DUPLEX  


def draw_box(Image, x, y, w, h):
    #na versão do python >=3 deve passar como parâmetro números inteiros. Erri:  error: (-5:Bad argument) in function 'line'
    cv2.line(Image, (x, y), (x + int((w/5)) ,y), (0,0,255), 2)
